...
```bash
git clone git@github.com:wu-uk/AutoYara.git
cd AutoYara
```
...
- `uv sync` 会按 `pyproject.toml` 和 `uv.lock` 安装项目依赖
- `--extra dev` 会额外安装测试与开发依赖（如 `pytest`、`ruff`、`black`）
...
IDA 模块使用 MCP 协议。你可以通过以下脚本启动 IDA MCP 服务器：

```bash
uv run python scripts/run_ida_server.py
```
